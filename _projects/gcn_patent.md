...
**中文：**  
当前拟以中国发明专利的形式申请，申请材料正在整理与准备中。该专利围绕一种面向深度学习可靠性提升的结构化噪声注入技术展开，重点保护其噪声生成机制、空间相关结构、均值保持正乘性门控形式，以及在神经网络中间表示中的注入方式与工程实现路径。

**English:**  
A China invention patent application is currently in preparation. The proposed patent concerns a structured noise-injection technology for improving reliability in deep learning, with protection focused on the noise-generation mechanism, spatial correlation structure, mean-preserving positive multiplicative gating, and the corresponding injection strategy and implementation pipeline in intermediate neural representations.
...
This is not merely a generic “noise-adding trick,” but a principled and implementable mechanism for structured noise design with a clear mathematical and engineering foundation. It has potential as a general reliability-enhancement module for deep learning, with relevance to calibration, distribution-shift robustness, uncertainty control, and deployment in reliability-critical settings.

---

## Current Scope / 当前拟保护范围
...
**中文：**  
本页面用于展示该专利方向的研究与申请准备情况。正式法律文本及最终权利要求以实际提交的专利申请文件为准。

**English:**  
This page is intended to present the research direction and filing status of the patent project. The final legal scope will be determined by the actual patent application documents as filed.
